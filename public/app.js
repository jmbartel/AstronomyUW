...
officersData.forEach((officer) => {
  ...
  card.innerHTML = `<div class="column is-one-third"><figure class="image"><img class="officer-image" src="placeholder-image.jpg" alt="${officer.name}"></figure></div><div class="officer-info column"><h2 class="title is-4">${officer.name}</h2><h3 class="subtitle is-5">${officer.title}</h3><p><strong>Year:</strong> ${officer.year}</p><p><strong>Major:</strong>${officer.major}</p><p>${officer.bio}</p></div>`;
